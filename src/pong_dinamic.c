...
int main() {
  ...
  nodelay(initscr(), 1);  //  не ожидает от пользователя ввода команды
  noecho();  //   не выводит введенный текст в консоли

  field(rack1_y, rack2_y, ball_x, ball_y, scr1, scr2);

  while ((scr1 < 21) && (scr2 < 21)) {
    // scanf("%c", &key1);
      key1 = getch();  //   перехват команды пользователя
...
void field(int rack1_y, int rack2_y, int ball_x, int ball_y, int scr1,
  ...
  char line = '|';
  clear();
    // printw("\33[0d\33[2J");   //   удаляет предыдущий кадр в коносли
  for (int i = 0; i < 25; i++) {
    for (int j = 0; j < 80; j++) {
      if (i == 0 || i == 24) {
        printw("%c", border);
